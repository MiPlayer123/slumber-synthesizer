...
export const PasswordResetTester = () => {
  ...
  const handleDirectPasswordReset = async () => {
    if (!isConfigured) {
      setErrorMessage(
        "Supabase client is not properly configured. Contact support.",
      );
      setStatus("error");
      return;
    }
    ...
    try {
      addLog(
        `Starting direct password reset with code: ${code.substring(0, 5)}...`,
      );
      ...
      if (sessionError) {
        ...
        if (
          ...
          sessionError.message.includes("code challenge does not match")
        ) {
          addLog(
            `Session exchange failed: This reset code has already been used or browser data was cleared`,
          );
          throw new Error(
            `This reset link can only be used once. Please request a new password reset link.`,
          );
        }
    ...
    return (
      <div className="space-y-6 max-w-md mx-auto">
        <Alert variant="destructive">
          ...
          <AlertDescription>
            The authentication service is not properly configured. This usually
            happens when environment variables are missing. Please contact
            support or check your application configuration.
          </AlertDescription>
        </Alert>

        <div className="p-6 bg-white dark:bg-gray-800 rounded-lg shadow text-center">
          <h2 className="text-xl font-semibold mb-4">
            Cannot Process Password Reset
          </h2>
          <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
            The application is missing essential configuration needed to process
            password resets.
          </p>
          <Button
    ...
    <div className="space-y-6 max-w-md mx-auto">
      <div className="space-y-4 p-6 bg-white dark:bg-gray-800 rounded-lg shadow">
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400">
          This tool allows you to test the password reset functionality directly
        ...
        <div className="space-y-3">
          <div className="space-y-2">
            ...
            <p className="text-xs text-gray-500">
              This is the code from your password reset link (after the ?code=
              part)
            </p>
          </div>
