...
export function VoiceRecorder({
  ...
  const startRecording = async () => {
    try {
      ...
      console.log(
        `Browser detection: iOS=${isIOS}, Safari=${isSafari}, Desktop=${isDesktop}`,
      );
    ...
    try {
      ...
      const processingTimeout = setTimeout(() => {
        ...
        toast({
          ...
          description:
            "The process took too long. Please try again with a shorter recording.",
        });
        setIsTranscribing(false);
      ...
      try {
        ...
        try {
          ...
          console.log(
            `Decoded audio: ${audioBuffer.duration}s, ${audioBuffer.numberOfChannels} channels, ${audioBuffer.sampleRate}Hz`,
          );
        } catch (decodeError) {
      ...
      toast({
        ...
        description:
          error.message || "Could not convert audio to compatible format",
      });
      setIsTranscribing(false);
    }
  };
  ...
  const transcribeAudio = async (audioBlob: Blob, fileExtension: string) => {
    try {
      ...
      console.log(
        `Preparing to transcribe audio: type=${mimeType}, extension=${fileExtension}`,
      );
      ...
      const base64Audio = await new Promise<string>((resolve, reject) => {
        reader.onloadend = () => {
          try {
            ...
            console.log(
              `Base64 conversion successful: ${base64Data.length} chars`,
            );
            resolve(base64Data);
          } catch (error) {
            reject(error);
      ...
      console.log(
        `Sending transcription request: format=${fileExtension}, size=${payload.dataSize} chars`,
      );
      ...
      if (error) {
        ...
        if (isDesktop) {
          throw new Error(
            "Desktop transcription failed. Try recording from a mobile device instead, or use the text input.",
          );
        } else {
          ...
          <div className="text-center">
            {isRecording ? (
              ...
              <div className="text-sm">
                Converting & transcribing your recording...
              </div>
            ) : (
              <div className="text-sm">
