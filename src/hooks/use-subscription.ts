...
export const useSubscription = () => {
  ...
  const logStatusOnce = (status: string, forceLog = false) => {
    ...
    const timeSinceLastLog =
      typeof lastNotificationRef.current === "number"
      ...
      try {
        ...
        const { data: subscriptionData, error: subscriptionError } =
          await supabase
            ...
            .select(
              "subscription_id, status, customer_portal_url, stripe_customer_id, cancel_at_period_end, current_period_end, canceled_at",
            )
            .eq("user_id", user.id)
            .maybeSingle();
        ...
        if (subscriptionError) {
          ...
          if (
            ...
            console.log(
              "Found active subscription with NULL subscription_id, attempting to fix...",
            );
            ...
            try {
              ...
              if (accessToken) {
                ...
                const fetchResponse = await fetch(
                  "/api/functions/v1/get-stripe-subscription",
                  {
                    method: "POST",
                    headers: {
                ...
                if (fetchResponse.ok) {
                  ...
                  if (stripeData.subscription_id) {
                    ...
                    if (updateError) {
                      console.error(
                        "Error updating NULL subscription_id:",
                        updateError,
                      );
                    } else {
                      console.log(
                        "Successfully updated NULL subscription_id in database",
                      );
              ...
              if (accessToken) {
                ...
                if (fetchResponse.ok) {
                  ...
                  if (
                    ...
                    isCanceling = true;
                    console.log(
                      "Detected canceled subscription still in paid period - treating as canceling",
                    );
                  }

                  // Determine the display status based on Stripe values
                  let displayStatus: "active" | "canceling" | "inactive" =
                    "inactive";
                  if (isActive) {
          ...
          console.log(
            "Found subscription with stripe_customer_id but null status, updating to active",
          );
  ...
  useEffect(() => {
    ...
    const checkReturnFromStripe = () => {
      ...
      if (tabParam === "subscription" && fromStripe === "true" && user) {
        console.log(
          "Detected return from Stripe portal, refreshing subscription data",
        );
        // Wait a moment to ensure page is fully loaded
        setTimeout(() => {
          refreshFromStripe().catch((err) => {
            console.error(
              "Error refreshing subscription after returning from Stripe:",
              err,
            );
          });
