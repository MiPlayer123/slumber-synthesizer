  ...
  return (
    <motion.header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled ? "py-3 bg-black/80 backdrop-blur-md border-b border-white/10" : "py-5"
      }`}
      initial={{ y: -100 }}
      animate={{ y: 0 }}
      ...
      <div className="container mx-auto px-6 flex justify-between items-center">
        ...
        <nav className="hidden md:flex items-center gap-8">
          {["Features", "Testimonials", "About", "Blog"].map((item, i) => (
            <motion.div key={i} whileHover={{ y: -2 }} transition={{ type: "spring", stiffness: 300, damping: 20 }}>
              <Link
          ...
          <div className="pl-4 flex items-center gap-3">
            <motion.div whileHover={{ scale: 1.05 }} transition={{ type: "spring", stiffness: 400, damping: 10 }}>
              <Button variant="ghost" className="text-sm text-white/90 hover:text-white hover:bg-white/10">
                Log in
              </Button>
            </motion.div>
            <motion.div whileHover={{ scale: 1.05 }} transition={{ type: "spring", stiffness: 400, damping: 10 }}>
