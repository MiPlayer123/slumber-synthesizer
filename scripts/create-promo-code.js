...
async function createCoupon() {
  ...
  const discountType = await question(
    "Discount type (1=percentage, 2=fixed amount): ",
  );

  let percent_off, amount_off, currency;

  if (discountType === "1") {
    percent_off = parseInt(
      await question("Percentage off (e.g., 25 for 25%): "),
    );
  } else {
    amount_off = parseInt(
      await question("Amount off in cents (e.g., 500 for $5.00): "),
    );
    currency = (await question("Currency (default: usd): ")) || "usd";
  }

  const duration =
    (await question("Duration (once/repeating/forever, default: once): ")) ||
    "once";
  ...
  const max_redemptions = await question(
    "Max redemptions (optional, press enter to skip): ",
  );
  ...
  try {
    const response = await fetch(
      `${SUPABASE_URL}/functions/v1/manage-promo-codes`,
      {
        method: "POST",
        headers: {
...
async function createPromotionCode(couponId) {
  ...
  const code = await question(
    'Promotion code (e.g., "BLACKFRIDAY25", leave empty for auto-generated): ',
  );
  const max_redemptions = await question(
    "Max redemptions for this code (optional): ",
  );
  const expires_at = await question("Expiry date (YYYY-MM-DD, optional): ");
  const first_time_transaction = await question(
    "First-time customers only? (y/n, default: n): ",
  );
  ...
  try {
    ...
    if (response.ok) {
      ...
      result.promotion_codes.forEach((code, index) => {
        ...
        if (code.expires_at) {
          console.log(
            `   Expires: ${new Date(code.expires_at * 1000).toLocaleDateString()}`,
          );
        }
      });
    } else {
  ...
  try {
    ...
    if (response.ok) {
      if (result.valid) {
        ...
        console.log(
          "Discount Info:",
          JSON.stringify(result.discount_info, null, 2),
        );
      } else {
...
async function main() {
  ...
  if (
    ...
    console.log(
      "⚠️  Please set your SUPABASE_URL and SUPABASE_ANON_KEY environment variables",
    );
