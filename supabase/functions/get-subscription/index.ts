...
serve(async (req) => {
  ...
  try {
    ...
    if (subscription.current_period_end) {
      ...
      try {
        ...
        if (refreshedSubscription.current_period_end) {
          currentPeriodEnd = new Date(
            refreshedSubscription.current_period_end * 1000,
          ).toISOString();
          console.log(
            `Retrieved current_period_end from fresh Stripe call: ${currentPeriodEnd}`,
          );
    ...
    if (
      ...
      console.log(
        `Setting cancelAtPeriodEnd=true for canceled subscription still in paid period until ${new Date(subscription.current_period_end * 1000).toISOString()}`,
      );
    }
