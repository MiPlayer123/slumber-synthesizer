...
serve(async (req) => {
  ...
  try {
    ...
    if (!dreamId || !description || !userId) {
      ...
      return new Response(
        JSON.stringify({
          error: "Dream ID, description, and user ID are required",
        }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    if (!base64ImageData) {
      ...
      const errorResponse = parseOpenAIError(
        "No image data found in response. Unsupported format.",
      );
    ...
    if (!imageBytes || imageBytes.length === 0) {
      ...
      return new Response(
        JSON.stringify({
          error: "Failed to process image data: Invalid binary data",
        }),
        {
          status: 500,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    console.log(
      ...
      Object.prototype.toString.call(fileBlob),
    );
    ...
    try {
      ...
      return new Response(
        JSON.stringify({
          error: `Error during upload: ${uploadCatchError.message || "Unknown error"}`,
        }),
        {
          status: 500,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
