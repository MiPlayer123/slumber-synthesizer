    ...
    try {
      ...
      if (session.mode === "subscription") {
        ...
        if (session.subscription) {
          ...
          try {
            ...
            logDebug(
              `Retrieved subscription details: status=${subscription.status}`,
            );
            ...
            if (
              ...
              if (verified && subscriptionId) {
                ...
                if (error) {
                  logDebug(
                    `Error upserting subscription record: ${error.message}`,
                    error,
                  );
                  ...
                  if (fallbackError) {
                    ...
                    logDebug(
                      "[verify-payment] Database updated with basic subscription details",
                    );
                  }
                } else {
                  logDebug(
                    "[verify-payment] Database should now reflect active subscription",
                  );
                }
              }
            }
            ...
            logDebug(
              `Error retrieving subscription: ${subscriptionError.message}`,
            );
          }
        }
      } else if (
      ...
      try {
        ...
        if (customerError) {
          ...
          logDebug(
            `Found customer ID in database: ${customerData.stripe_customer_id}`,
          );

          // Check that the session's customer matches our records
          if (
            session.customer &&
            customerData.stripe_customer_id !== session.customer
          ) {
            logDebug(
              `Customer mismatch: ${customerData.stripe_customer_id} vs ${session.customer}`,
            );
          ...
          logDebug(
            `No existing customer record found. Creating new record with customer ID ${session.customer}`,
          );
          ...
          try {
            ...
            if (insertError) {
              logDebug(
                `Error creating customer record: ${insertError.message}`,
              );
            } else {
