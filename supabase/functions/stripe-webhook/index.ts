  ...
  try {
    ...
    switch (event.type) {
      case "checkout.session.completed": {
        ...
        let userId = session.metadata?.user_id || (session.subscription_data?.metadata?.user_id as string);
        ...
        if (!userId) {
          logDebug("No user ID in checkout session metadata, trying to find from customer metadata");
          // Try to get the user ID from the customer metadata
          try {
            const customer = await stripe.customers.retrieve(customerId);
            if (customer && !customer.deleted && customer.metadata?.user_id) {
              logDebug(`Found user_id ${customer.metadata.user_id} in customer metadata`);
        ...
        if (customerError || !customerData) {
          ...
          if (insertError) {
            ...
            logDebug(`Successfully created subscription record with subscription_id=${subscriptionId}`);
          }
        ...
        if (updateError) {
          ...
          if (verifyError) {
            ...
            logDebug(`Verified database update, subscription_id is now: ${verifyData.subscription_id || 'NULL'}, status: ${verifyData.status}`);
          }
        }
      ...
      case "invoice.paid": {
        ...
        if (subscription.status === 'active') {
          ...
          if (!customerData.subscription_id || customerData.status !== 'active') {
            ...
            if (updateError) {
              ...
              if (fallbackError) {
                logDebug("Error with fallback update after payment", fallbackError);
              } else {
                logDebug("Successfully updated basic subscription info after payment");
              }
            } else {
              logDebug("Successfully updated subscription status after payment");
            }
          } else {
            logDebug("Subscription already active and up-to-date, no database update needed");
          }
        } else {
          logDebug(`Not updating subscription because status is: ${subscription.status}`);
        }
