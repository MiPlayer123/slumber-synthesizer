...
serve(async (req) => {
  ...
  try {
    ...
    const enhancedDescription = dreamDescription.length > 10 
      ? dreamDescription 
    ...
    if (!geminiApiKey) {
      ...
      return new Response(
        JSON.stringify({ 
          ...
          message: 'The image generation service is not properly configured. Please contact support.'
        }),
        { 
          status: 500,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
    ...
    const imageResponse = await fetch('https://generativelanguage.googleapis.com/v1beta/models/imagen-3.0-generate-002:predict?key=' + geminiApiKey, {
      method: 'POST',
      headers: {
    ...
    if (!imageResponse.ok) {
      ...
      return new Response(
        JSON.stringify({ 
          ...
          details: errorText,
          message: 'The fallback image generation service encountered an error.'
        }),
        { 
          status: 500,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
    ...
    if (!base64ImageData) {
      console.error('Response format (fallback):', JSON.stringify(imageData).substring(0, 200) + '...');
      return new Response(
        JSON.stringify({ 
          error: 'No image data found in response', 
          details: 'Unsupported format',
          message: 'The fallback image generation service returned an unexpected format.'
        }),
        { 
          status: 500,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
