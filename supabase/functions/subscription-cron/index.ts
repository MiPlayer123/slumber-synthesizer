...
const logDebug = (message: string, data?: any) => {
  console.log(`[subscription-cron] ${message}`, data ? JSON.stringify(data) : "");
};
...
serve(async (req) => {
  try {
    ...
    const { data: expiredSubscriptions, error: queryError } = await supabase
      ...
      .select("id, user_id, subscription_id, subscription_status, current_period_end, cancel_at_period_end")
      .or(`subscription_status.eq.canceling,and(subscription_status.eq.active,cancel_at_period_end.eq.true)`)
    ...
    logDebug(`Found ${expiredSubscriptions.length} expired subscriptions to process`);
    
    // Process each expired subscription
    const results = await Promise.all(
      expiredSubscriptions.map(async (subscription) => {
        logDebug(`Processing expired subscription for user ${subscription.user_id}`);
        ...
        if (updateError) {
          logDebug(`Error updating subscription ${subscription.id}`, updateError);
          return { id: subscription.id, success: false, error: updateError };
        } else {
          logDebug(`Successfully transitioned subscription ${subscription.id} to canceled status`);
